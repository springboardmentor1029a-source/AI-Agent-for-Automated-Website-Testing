    ...
    def _parse_instruction(self, state: AgentState) -> AgentState:
        ...
        try:
            ...
            if "quota" in error_str.lower() or "429" in error_str or "insufficient_quota" in error_str:
                ...
                state["error"] = f"Error parsing instruction: {str(e)}"
                state["parsed_steps"] = []
    ...
    def _validate_page(self, instruction: str) -> list:
        ...
        try:
            ...
            if "search" in instruction_lower or "find" in instruction_lower:
                search_inputs = self.page.locator("input[type='search'], input[name*='search'], input[id*='search'], #twotabsearchtextbox").count()
                if search_inputs > 0:
                    validations.append({
                        "type": "search_box",
                        "status": "pass",
    ...
    def _execute_playwright_code(self, state: AgentState) -> AgentState:
        ...
        try:
            ...
            try:
                ...
                if "Execution context was destroyed" in error_msg or "navigation" in error_msg.lower() or "Target closed" in error_msg:
